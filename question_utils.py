...
def normalize_topic(topic: str) -> str:
    """
    Normalizes a topic string to lowercase, removes punctuation, and applies aliases.
...
def cluster_embeddings(embeddings: np.ndarray, k: int, method: str = 'kmeans') -> List[List[int]]:
    """
    ...
    try:
        ...
        logger.debug(f"Clustered {len(embeddings)} embeddings into {len(clusters)} clusters")
        return clusters
...
def deduplicate_by_similarity(items: List[Dict], threshold: float = 0.92, embedding_key: str = 'embedding') -> List[Dict]:
    """
    ...
    for i in range(len(items)):
        ...
        for j in range(i + 1, len(items)):
            ...
            if sim >= threshold:
                # Mark as duplicate
                keep[j] = False
                logger.debug(f"Marked item {j} as duplicate of item {i} (similarity: {sim:.3f})")
    
    result = [items[i] for i in range(len(items)) if keep[i]]
    logger.info(f"Deduplicated {len(items)} items to {len(result)} items (threshold: {threshold})")
...
def merge_chunks_by_budget(chunks: List[Dict], token_budget: int = 1200, sort_by: str = 'chunk_index') -> str:
    """
...
def select_diverse_questions(questions: List[Dict], n: int, input_topics: List[str]) -> List[Dict]:
    """
    Selects n diverse questions from a larger pool.
    Prioritizes topic coverage and diversity.
    ...
    logger.info(f"Selected {len(selected)} diverse questions from pool of {len(questions)}")
    return selected
