...
Implements the core question generation logic with Qdrant retrieval,
clustering, LLM generation, and MongoDB caching.
"""
import time
import json
import numpy as np
...
from question_utils import (
    normalize_topics, cluster_embeddings, deduplicate_by_similarity,
    merge_chunks_by_budget, compute_overlap_count, select_diverse_questions
)
...
class QuestionGenerationService:
    def __init__(self, qdrant_db: Optional[QdrantDB] = None, 
    ...
    def generate_questions_for_topic_list(self, 
                                         class_id: str, 
                                         chapter_id: str, 
        ...
        """
        logger.info(f"Generating {n} questions for doc={class_id}, chapter={chapter_id}, topics={input_topics}, mode={mode}")
        ...
        cached_questions = self._check_cache(class_id, chapter_id, normalized_topics, n, mode)
        ...
        for cluster_idx, cluster_indices in enumerate(clusters):
            ...
            context = merge_chunks_by_budget(cluster_chunks, self.token_budget_per_cluster)
            
            # Generate questions
            questions = self._llm_generate_questions(context, normalized_topics, self.questions_per_cluster)
        ...
        # Step 7: Deduplicate
        unique_questions = deduplicate_by_similarity(generated_questions, self.dedupe_threshold)
        logger.info(f"After deduplication: {len(unique_questions)} questions")
        
        # Step 8: Select top N diverse questions
        selected_questions = select_diverse_questions(unique_questions, n, normalized_topics)
        ...
        for q in selected_questions:
            ...
            q['status'] = 'draft'
            q['origin'] = 'generated'
            q['overlap_count'] = compute_overlap_count(q.get('topic_keys', []), normalized_topics)
            ...
            try:
                ...
                logger.error(f"Failed to insert question to MongoDB: {e}")
        
        elapsed = time.time() - start_time
        logger.info(f"Question generation completed in {elapsed:.2f}s, returning {len(selected_questions)} questions")
        
        return selected_questions
    
    def _check_cache(self, class_id: str, chapter_id: str, topics: List[str], n: int, mode: str) -> List[Dict]:
        """
    ...
    def _query_qdrant_for_topics(self, class_id: str, chapter_id: str, 
                                 topics: List[str], collection_name: str) -> List[Dict]:
        """
        ...
        # Build filter
        must_conditions = [
            models.FieldCondition(key='class_id', match=models.MatchValue(value=class_id)),
            models.FieldCondition(key='chapter_id', match=models.MatchValue(value=chapter_id))
        ]
        
        should_conditions = [
            models.FieldCondition(key='topic_keys', match=models.MatchValue(value=topic))
            for topic in topics
        ]
    ...
    def _llm_generate_questions(self, context: str, topics: List[str], per_cluster: int = 2) -> List[Dict]:
        """
...
JSON array:"""
    ...
    def _compute_question_embedding(self, question_text: str) -> List[float]:
        ...
        try:
            ...
            if hasattr(self.qdrant, '_model_type') and self.qdrant._model_type == 'fastembed':
